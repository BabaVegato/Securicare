#include <touchgfx/Font.hpp>

#ifndef NO_USING_NAMESPACE_TOUCHGFX
using namespace touchgfx;
#endif

FONT_LOCATION_FLASH_PRAGMA
KEEP extern const touchgfx::KerningNode kerning_timesbd_27_4bpp[] FONT_LOCATION_FLASH_ATTRIBUTE =
{
    {0x0041, -1},  // (First char = [0x0041, ], Second char = [0x0020, ], Kerning dist = -1) 
    {0x0046, -1},  // (First char = [0x0046, ], Second char = [0x0020, ], Kerning dist = -1) 
    {0x0031, -1},  // (First char = [0x0031, ], Second char = [0x0031, ], Kerning dist = -1) 
    {0x0054, -2},  // (First char = [0x0054, ], Second char = [0x003A, ], Kerning dist = -2) 
    {0x0020, -1},  // (First char = [0x0020, ], Second char = [0x0041, ], Kerning dist = -1) 
    {0x0046, -2},  // (First char = [0x0046, ], Second char = [0x0041, ], Kerning dist = -2) 
    {0x0054, -2},  // (First char = [0x0054, ], Second char = [0x0041, ], Kerning dist = -2) 
    {0x0041, -2},  // (First char = [0x0041, ], Second char = [0x0054, ], Kerning dist = -2) 
    {0x0054, -2},  // (First char = [0x0054, ], Second char = [0x0061, ], Kerning dist = -2) 
    {0x0054, -2},  // (First char = [0x0054, ], Second char = [0x0063, ], Kerning dist = -2) 
    {0x0054, -2},  // (First char = [0x0054, ], Second char = [0x0065, ], Kerning dist = -2) 
    {0x0054, -2},  // (First char = [0x0054, ], Second char = [0x006F, ], Kerning dist = -2) 
    {0x0054, -2},  // (First char = [0x0054, ], Second char = [0x0072, ], Kerning dist = -2) 
    {0x0054, -2},  // (First char = [0x0054, ], Second char = [0x0073, ], Kerning dist = -2) 
    {0x0054, -2},  // (First char = [0x0054, ], Second char = [0x0075, ], Kerning dist = -2) 
};

